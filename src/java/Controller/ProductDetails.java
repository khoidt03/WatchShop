...
public class ProductDetails extends HttpServlet {
    ...
    throws ServletException, IOException { 
        ...
        Cart c = new Cart(); 
        String pID = request.getParameter("pdetails_id"); 
        int quantityCart = 0; 
        Product p = dao.getProductByID(pID);  
        if(c.getItemByID(pID) != null){
            quantityCart = c.getItemByID(pID).getQuantity(); 
        } 
        request.setAttribute("quantityCart", quantityCart);
        request.setAttribute("cartSize", c.getAllItem().size());   
        request.setAttribute("ProductByCategory", dao.getProductByCategory(p.getCategory().getCategory_id())); 
